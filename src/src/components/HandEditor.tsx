...
export function HandEditor({ hand, discardsBySeat, meldsBySeat, analysisHandLength, shanten, waits, historyLength, onRemove, onRemoveDiscard, onRemoveMeldTile, onUndo, onReset }: Props) {
  ...
  return <section className="panel input-panel">
    ...
    <div className="input-meta"><span className={hand.length === analysisHandLength ? 'valid' : ''}>{hand.length === analysisHandLength ? '✓ 解析準備完了' : `あと${analysisHandLength - hand.length}枚で解析できます`}</span><code>{displayHand.map((id) => tileById(id).short).join(' ') || '—'}</code></div>
  </section>
}

...
export function HandEditor({ hand, recommendedDiscards = [], discardsBySeat, meldsBySeat, riichiBySeat, analysisHandLength, shanten, waits, historyLength, sampleMode, browserStatus, onRemove, onRemoveDiscard, onRemoveMeldTile, onUndo, onReset, onToggleSample, onConnect }: Props) {
  ...
  return <section className="panel input-panel">
    ...
    <div className="connection-actions"><button className={`sample-button ${sampleMode ? 'active' : ''}`} onClick={onToggleSample}>{sampleMode ? 'サンプル非表示' : 'サンプル表示'}</button><button className={`connect-button ${browserStatus}`} onClick={() => void onConnect()} disabled={browserStatus === 'connecting'}>{browserStatus === 'connected' && !sampleMode ? '接続済み' : '接続する'}</button></div>
  </section>
}

...
export function AnalysisResults({ handLength, expectedHandLength, analysis }: Props) {
  return <section className="panel results-panel">
    ...
    {analysis.length ? <div className="result-list">{analysis.map((result, index) => { const tile = TILES[result.discard]; const tieGroup = tieGroupFor(analysis, index); const tieClass = tieGroup === null ? '' : `tie-group-${tieGroup}`; return <article className={`result-card ${index === 0 ? 'best' : ''} ${tieClass}`} key={tile.id}><div className="rank">{String(rankFor(analysis, index)).padStart(2, '0')}</div><div className={`result-tile tile tile-small suit-${tile.suit}`}><img className="tile-art" src={`/tiles/${tile.asset}`} alt="" /></div><div className="result-main"><div className="result-title"><strong>{index === 0 ? 'おすすめ' : '候補'}</strong><span>{result.shanten === -1 ? '和了形' : `${result.shanten}シャンテン`}</span></div><div className="result-stats"><span>山に残り: <b>{result.effectiveTileCount}枚</b></span></div></div><div className="result-arrow">→</div></article> })}</div> : <div className="analysis-empty" />}
  </section>
}
